...
import com.oneconnect.demoapp.adapter.FreeServerAdapter
import top.oneconnectapi.app.OneConnectServer

class FreeServersFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        servers =
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                requireActivity().intent.getParcelableArrayListExtra("freeServers", OneConnectServer::class.java)
            } else {
                @Suppress("DEPRECATION")
                requireActivity().intent.getParcelableArrayListExtra("freeServers")
            }
    }
    ...
    override fun onCreateView(
        ...
        servers?.let {
            val adapter = FreeServerAdapter(servers!!) { selectedServer ->
